...
for ip in ${node_ip}
do
   ssh root@${ip} "yum -y install conntrack ipvsadm ipset jq sysstat curl iptables libseccomp curl || yum -y install conntrack ipvsadm ipset jq sysstat curl iptables libseccomp curl"
done
for ip in ${node_ip}
do
